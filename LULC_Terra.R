...
pred_aoa_es<-tm_shape(prediction_aoa) +
  ...
  tm_scale_bar(bg.color = "white", bg.alpha = 0.75) +
  tm_layout(legend.bg.color = "white", legend.bg.alpha = 0.75) +
  tm_add_legend(type = "fill", labels = c("AOA = 0"), col = "grey", alpha = 0.8)  # Custom legend entry
...
# Select predictors (numeric columns)
predictors <- extr[, c("B2", "B3", "B4", "B8", "B6", "B7", "B11", "NDVI", "NDVI_sd_5")]

# Ensure 'Label' is a factor and correctly ordered
extr$Label <- factor(extr$Label, levels = c("Fallow Land", "Forest", "Grassland", "Industrial", "Planted Field", "Urban", "Water body"))
...
featurePlot(
  ...
  auto.key = list(
    ...
    columns = 1,
    points = TRUE,
    text = levels(extr$Label)
    
  ),
  key = list(
    text = list(levels(extr$Label)),
    points = list(fill = cols, pch = 21) # Use pch = 21 and specify fill
  ), 
  pch = 21,                      # Set the plot symbols to filled circles
  fill = cols   
)
...
# Define color palette for 7 classes
cols <- c("yellow", "darkgreen", "lightgreen", "darkorange", "brown", "red", "blue")
# Select only the desired bands (B2, B3, and B4)
predictors_subset <- extr[, c("B2", "B3", "B4")]
...
cols <- c("yellow", "darkgreen", "lightgreen", "darkorange", "brown", "red", "blue")
# Select only the desired bands (B2, B3, and B4)
predictors_subset <- extr[, c("B6", "B7", "B8")]
...
cols <- c("yellow", "darkgreen", "lightgreen", "darkorange", "brown", "red", "blue")
# Select only the desired bands (B2, B3, and B4)
predictors_subset <- extr[, c("B11", "NDVI", "NDVI_sd_5")]
...
# Select predictors (numeric columns)
predictor <- extr_es[, c("B2", "B3", "B4", "B8", "B6", "B7", "B11", "NDVI", "NDVI_sd_5")]

# Ensure 'Label' is a factor and correctly ordered
extr_es$Label <- factor(extr_es$Label, levels = c("Fallow Land", "Forest", "Grassland", "Industrial", "Planted Field", "Urban", "Water body"))
...
featurePlot(
  ...
  auto.key = list(
    ...
    columns = 1,
    points = TRUE,
    text = levels(extr_es$Label)
    
  ),
  key = list(
    text = list(levels(extr_es$Label)),
    points = list(fill = cols, pch = 21) # Use pch = 21 and specify fill
  ), 
  pch = 21,                      # Set the plot symbols to filled circles
  fill = cols   
...
cols <- c("yellow", "darkgreen", "lightgreen", "darkorange", "brown", "red", "blue")
# Select only the desired bands (B2, B3, and B4)
predictor_subset <- extr_es[, c("B2", "B3", "B4")]
...
cols <- c("yellow", "darkgreen", "lightgreen", "darkorange", "brown", "red", "blue")
# Select only the desired bands (B2, B3, and B4)
predictor_subset <- extr_es[, c("B6", "B7", "B8")]
...
cols <- c("yellow", "darkgreen", "lightgreen", "darkorange", "brown", "red", "blue")
# Select only the desired bands (B2, B3, and B4)
predictor_subset <- extr_es[, c("B11", "NDVI", "NDVI_sd_5")]
